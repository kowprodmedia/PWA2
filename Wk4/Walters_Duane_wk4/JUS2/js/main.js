...
!function (e) {
    ...
    $(".banner").cycle({slides: ".slide", fx: "scrollHorz", speed: 800, timeout: 4e3, prev: "#prev", next: "#next"})
});
/*===== Created by duaneawalters on 3/17/13 =====*/


/*===== End Site Staple Functions =====*/	
...
$('.masterTooltip').hover(function(){
	...
	var mouseY = e.pagex + 10; //Get Y coordinates
	$('.tooltip')
...
/*
	$('ul.tabs-nav').each(function(){
		//Keeps track of each set of tabs
		//Active tab and associated content
	var $active, $content, $links = $(this).find('a');
	
		//Uses matching location.hash as active link
		//If no match, uses first link as active
	$active = $($links.filter('[href="'+location.hash+'"]')[0] || $links[0]);
	$active.addClass('active');
	
	$content = $($active[0].hash);
		
		//Hides inactive tabs' content
	$links.not($active).each(function () {
		$(this.hash).hide();
	});
		
		//Binds click even handler
	$(this).on('click', 'a', function(e){
		//Makes old tab inactive
	$active.removeClass('active');
	$content.hide();
	
		//Updates variables with new link and content
	$active = $(this);
	$content = $(this.hash);
	...
	$content.show();
		...
		//Prevents the anchor's default click action
	e.preventDefault();
	});
});
*/
...
/*===== Go to "dashboard" page (tried to get to "user_update" but was unable.=====*/
...
$.getJSON("xhr/update_user.php", function(data){
	console.log(data);
	$.each(data, function(key, val){
		console.log(val.first_name);
		$(".updateuser").html("Welcome User: " + val.first_name);
	})
});*/
	...
	$.ajax({
		...
		success: function(response){
			if(response.error){
				...
				for(var i=0, j=response.users.length; i < j; i++){
					var result = response.users[i];
					
					$(".users").append(
					//'<div style="border:1px solid black">'+
					'<div id="current_users" class="ui-state-default">'+
					" <input class='projectid' type='hidden' value='" + result.id + "'>"+
					" Project Name: " + result.projectName + "<br>" +
					" Project Due Date: " + result.dueDate + "<br>" +
					" Project Description: " + result.projectDescription + "<br>" +
...
/*========== Update project function: Get Projects ==========*/
